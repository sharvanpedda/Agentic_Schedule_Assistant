...
async def lifespan(app: FastAPI):
    ...
    start_scheduler()
    llm = get_llm()
    print(f"[schedule-assistant] LLM: {llm.provider or 'none'} "
          f"(connected={llm.connected}) — fallback parser active" if not llm.connected
          else f"[schedule-assistant] LLM: {llm.provider} ({llm.model})")
    yield
    # --- shutdown ---
...
app = FastAPI(title="Agentic RAG Schedule Assistant", version="1.0.0", lifespan=lifespan)
