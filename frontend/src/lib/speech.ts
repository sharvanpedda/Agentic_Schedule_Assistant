...
export function startListening(
  ...
  rec.onresult = (e: any) => {
    // Iterate through results from end to start to find the best final result
    let bestText = "";
    ...
    for (let resultIdx = e.results.length - 1; resultIdx >= 0; resultIdx--) {
      ...
      // Skip interim results if we've already found a final one
      if (!result.isFinal && bestText) continue;
      
      // Iterate through alternatives in this result
      ...
      // If this is a final result, stop looking at earlier results
    ...
    if (bestText) {
      ...
      onResult(bestText);
    } else if (e.results.length > 0 && !e.results[e.results.length - 1].isFinal) {
      // Interim result but no text yet — wait for more speech
    } else {
      console.warn("[Voice Input] No text captured from results:", e.results);
